...
# 设定文件夹路径  
data_folder = 'path_to_your_data_folder'  # 请替换为您的数据文件夹路径  
...
barcodes_files = [f for f in file_list if f.endswith('_barcodes.tsv')]  # 假设barcodes文件以_barcodes.tsv结尾  
...
for mtx_file, barcodes_file in zip(matrix_files_sorted, barcodes_files_sorted):  
    ...
    matrix = mmread(os.path.join(data_folder, mtx_file)).toarray()  
